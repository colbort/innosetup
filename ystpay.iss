...
#define strAppName "赢商通"#define strAppVers "3.1"
#define strInstallName "ystpay"
#define strUnInstallName "卸载赢商通"
...
begin
    ...
    begin
        if MsgBox('删除安装目录及所有文件。', mbConfirmation, MB_YESNO) = IDYES then
        begin
...
begin
    if (IsWin64()) then begin
        SubKeyName := 'Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\{F68808C4-8F03-443D-BDAB-C1662F69C910}_is1';
        RegWriteStringValue(HKLM64,SubKeyName,'DisplayIcon',iconPath);
    end else begin
        SubKeyName := 'Software\Microsoft\Windows\CurrentVersion\Uninstall\{F68808C4-8F03-443D-BDAB-C1662F69C910}_is1';
        RegWriteStringValue(HKLM,SubKeyName,'DisplayIcon',iconPath);
    end;
end;

//安装图像识别软件
function SetUpOCR(): Boolean;
var       
  Version : String;  
  ResultCode: Integer;begin
  if IsWin64 then
  begin
    RegQueryStringValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Tesseract-OCR', 'DisplayVersion', Version);
  end else begin
    RegQueryStringValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Tesseract-OCR', 'DisplayVersion', Version);
  end
  if Version <> '3.02.02' then 
  begin
    Exec(ExpandConstant('{app}\tesseract\tesseract-ocr-setup-3.02.02.exe'), '/S', '', SW_HIDE, ewWaitUntilTerminated, ResultCode)
  end ;
end; 
...
begin     
  ...
  begin
    SetUninstallIcon(ExpandConstant('{app}\ystpay.ico'));
    SetUpOCR();
  end;
end; 

[Setup]
AppId = {{F68808C4-8F03-443D-BDAB-C1662F69C910}
AppName = {#strAppName}
AppVersion = {#strAppVers}
DefaultDirName = "D:\{#strAppName}"
OutputBaseFilename = {#strInstallName}(v{#strAppVers})SetupIconFile = {#strInstallName}.ico
DefaultGroupName = {#strAppName}//PrivilegesRequired = admin
DisableWelcomePage = no


[Files]
;主程序必须安装的部分
Source: "ystpay\*.*"; DestDir: "{app}"Source: "ystpay\imageformats\*.*"; DestDir: "{app}\imageformats"
Source: "ystpay\sqldrivers\*.*"; DestDir: "{app}\sqldrivers"
Source: "ystpay\data\*.*"; DestDir: "{app}\data"
Source: "ystpay\Resources\*.*"; DestDir: "{app}\Resources"
Source: "ystpay\tesseract\*.*"; DestDir: "{app}\tesseract"; Flags:overwritereadonly deleteafterinstall

[Tasks]Name: "desktopicon"; Description: "{cm:CreateDesktopIcon}"; GroupDescription: "{cm:AdditionalIcons}" 
[Icons]
Name: "{group}\{#strAppName}"; Filename: "{app}\{#strInstallName}.exe"
Name: "{group}\{#strUnInstallName}"; Filename: "{uninstallexe}"Name: "{commondesktop}\{#strAppName}"; Filename: "{app}\{#strInstallName}.exe"; Tasks: desktopicon

[Run]
;安装图像识别库
Filename: "{app}\ystpay.exe"; Description:"执行应用程序"; Flags:postinstall
