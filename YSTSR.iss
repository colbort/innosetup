#define strAppName "赢商通智慧零售"
#define strAppVers "1.05"
#define strInstallName "YSTSR"
#define strUnInstallName "卸载赢商通智慧零售"
...
begin
    if (IsWin64()) then begin
        SubKeyName := 'Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\{369D9E50-8D89-45DB-AB4B-2251182FE00C}_is1';
        RegWriteStringValue(HKLM64,SubKeyName,'DisplayIcon',iconPath);
    end else begin
        SubKeyName := 'Software\Microsoft\Windows\CurrentVersion\Uninstall\{369D9E50-8D89-45DB-AB4B-2251182FE00C}_is1';
...
begin
    ...
    begin
        SetUninstallIcon(ExpandConstant('{app}\YSTSR.ico'));
    end;
end;


[Setup]
AppId = {{369D9E50-8D89-45DB-AB4B-2251182FE00C}
AppName = {#strAppName}
AppVersion = {#strAppVers}
DefaultDirName = "D:\{#strAppName}"
OutputBaseFilename = YSTSR(v{#strAppVers})
SetupIconFile = YSTSR.ico
...
[Files]
;主程序必须安装的部分
Source: "YSTSR\*.*"; DestDir: "{app}"
Source: "YSTSR\imageformats\*.*"; DestDir: "{app}\imageformats"
Source: "YSTSR\sqldrivers\*.*"; DestDir: "{app}\sqldrivers"
Source: "YSTSR\data\*.*"; DestDir: "{app}\data"
...
[Run]
Filename: "{app}\YSTSR.exe"; Description:"执行应用程序"; Flags:postinstall
